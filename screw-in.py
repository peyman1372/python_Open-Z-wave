...
while True:
    ...
    if mydata == "1":
        for node in network.nodes.itervalues():
            node.set_switch(72057594126614528, True)


    elif mydata == "0":
        for node in network.nodes.itervalues():
            node.set_switch(72057594126614528, False)
    elif mydata == "2":
        for node in network.nodes:
            for val in network.nodes[node].get_sensors() :
                if network.nodes[node].node_id==4:
                    print("  %s %s %s" % (network.nodes[node].values[val].label,network.nodes[node].get_sensor_value(val), network.nodes[node].values[val].units))
